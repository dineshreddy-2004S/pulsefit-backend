...
const registerWithOtp = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO users (
        name, email, password, plain_password, role, status, 
        phone, gym_name, gym_logo, gym_address, city, state, pincode, gst_number
      ) VALUES (?, ?, ?, ?, ?, 'PENDING', ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        name, email, hashedPassword, password, assignedRole,
        phone || null,
    ...
    res.status(201).json({ message: 'Account registered and email verified! Awaiting Super Admin approval.' });
  } catch (error) {
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (user.status !== 'APPROVED') {
      return res.status(403).json({ message: 'Account is pending approval from Administrator / Gym Owner.' });
    }
    ...
    const token = jwt.sign(
      ...
      process.env.JWT_SECRET || 'supersecretgymkey2026',
      { expiresIn: '12h' }
    );
...
// 4. Current Authenticated Profile (/api/auth/me)
...
// 5. Send Forgot Password Reset Link via Email
const forgotPasswordWithLink = async (req, res) => {
  ...
  try {
    ...
    console.log(`✅ Reset link email sent successfully to: ${user.email}`);

    res.json({ message: 'A secure password reset link has been dispatched to your email!' });
  } catch (error) {
    console.error('🔥 Error in forgotPasswordWithLink:', error);
    res.status(500).json({ message: 'Failed to send reset link: ' + error.message });
  }
};

// 6. Reset Password via Link Token
const resetPasswordWithToken = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE users SET password = ?, plain_password = ?, reset_token = NULL, reset_token_expires = NULL WHERE id = ?',
      [hashedPassword, newPassword, users[0].id]
    );

    res.json({ message: 'Password successfully updated! You may now sign in with your new password.' });
  } catch (error) {
