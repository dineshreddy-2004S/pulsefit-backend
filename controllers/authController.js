...
const sendRegistrationOtp = async (req, res) => {
  ...
  try {
    ...
    sendRegistrationOTP(email, otp, name)
      .then(() => console.log(`✅ Registration OTP delivered to ${email}`))
      .catch((err) => console.error(`❌ Background OTP delivery failed:`, err.message));
  ...
  try {
    ...
    await pool.query(
      ...
      [
        ...
        assignedRole,
        phone || null,
        assignedRole === 'GYM_OWNER' ? (gym_name || 'Pulse Fit Facility') : null,
  ...
  try {
    ...
    if (user.gym_owner_id) {
      const [ownerRows] = await pool.query(
        'SELECT name, gym_name, gym_logo FROM users WHERE id = ?',
        [user.gym_owner_id]
      );
...
// 5. Send Password Reset Link (Asynchronous & Fast Dispatch)
const forgotPasswordWithLink = async (req, res) => {
  ...
  try {
    ...
    sendPasswordResetEmail(user.email, resetUrl, user.name)
      .then(() => console.log(`✅ Reset link email delivered to ${user.email}`))
      .catch((err) => console.error(`❌ Background email delivery failed:`, err.message));
